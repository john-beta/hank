...
- Avoid proposals for a workspace that already is well organized. If the explorations reveal that the workspace is genuinely aligned with the curation criteria, you can abstain from proposing new redundant structures.
- Don't do a one-shot exploration. Explore the workspace iteratively instead: start always listing the workspace tree, then run reasonably sized follow-up explorations based on what the tree reveals. Size each follow-up by how much it will return rather than by a fixed unit, so each step makes progress while staying digestible.
